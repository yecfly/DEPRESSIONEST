...
def savefacepatchmodelwithonlytrainablevariables(TrainID, sess, saver2, FPI):
    if TrainID%100>30:
        if FPI==1:
            saver2.save(sess, './FPPTM/EyePatch_TrainonD502_TestonD531_N4_R2_20171017210026_only_trainable_variables.ckpt')#TestAccuracy:0.60510204	FinalLoss:1.5593829036
        elif FPI==2:
            saver2.save(sess, './FPPTM/MiddlePatch_TrainonD502_TestonD531_N4_R12_20171018061740_only_trainable_variables.ckpt')#TestAccuracy:0.47959184	FinalLoss:1.6764027119
        elif FPI==3:
            saver2.save(sess, './FPPTM/MouthPatch_TrainonD502_TestonD531_N4_R14_20171018053755_only_trainable_variables.ckpt')#TestAccuracy:0.59285714	FinalLoss:1.5720846534
    elif TrainID%100<20:
        if FPI==1:
            saver2.save(sess, './FPPTM/')
        elif FPI==2:
            saver2.save(sess, './FPPTM/')
        elif FPI==3:
            saver2.save(sess, './FPPTM/')
    ...
    try:
        ...
        if FacePatchID==1:
            ...
            if NetworkType==4:#4
                ...
                net=tflearn.conv_2d(net, 32, 3, activation='relu', name='eye_conv2_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='eye_pool2')
                efc2 = tflearn.fully_connected(net, 1024, activation='tanh', name='eye_fc2')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='eye_conv3_1_3x3')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='eye_conv3_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='eye_pool3')
                net=tflearn.fully_connected(net, 1024, activation='tanh', name='eye_fc1')
                net=tf.concat([net, efc2], 1, name='eye_fc')
                net=tflearn.dropout(net, 0.8)
            else:
                print("UNVALID INPUT ")
                exit(-1)
            ...
            saver=setSavertoTrainableVariables(tf.get_default_graph())
            ibs=batchSize
            with tf.Session() as sess:
                ...
                ##savegraph(sess, saver, saver2, FacePatchID)
                #exit(-1)

                iters=int((traindata.num_examples*Epoch)/batchSize)+1
                for i in range(iters):
                    batch=traindata.next_batch(batchSize, shuffle=False)
                    _, tloss=sess.run([optimizer, loss], feed_dict={x:batch[2],y:batch[5]})
                    if tloss<mini_loss:
                        mini_loss=tloss
                    v_accuracy, valid_loss, oaa, confu_mat = Valid_on_TestSet(sess, accuracy, sum_test, loss, softmax,
                                                              x, testdata.eyep, y, testdata.labels)
                    ...
                    print("CLR:%.8f Ite:%06d Bs:%03d Epo:%04d Los:%.8f mLo:%08f\tVALID>> mVL: %.8f\tVL: %.8f\tVA: %f\tOAA: %f\tT: %fs"%
                          (clr,i,batchSize,traindata.epochs_completed, tloss, mini_loss, loss_a.minimun_loss, valid_loss, v_accuracy, oaa, (tt-t1)))
                    if laflag:
                        file_record = logfile(file_record, runs=runs, OAA=oaa, valid_loss=valid_loss, valid_min_loss=loss_a.minimun_loss, 
                            final_train_loss=tloss, train_min_loss=mini_loss,
                            TA=v_accuracy, TC=(tt-t1),ILR=ilr, FLR=clr, LS=lrstep, ites=i,
                            Epo=traindata.epochs_completed, cBS=batchSize, iBS=ibs,
                            input=sys.argv, CM=confu_mat, T=time.localtime(tt), 
                            df=Dataset_Dictionary.get(trainpklID)+'&&'+Dataset_Dictionary.get(testpklID))
        ...
        #
            ...
            if NetworkType==4:#4
                ...
                net=tflearn.conv_2d(net, 32, 3, activation='relu', name='middle_conv2_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='middle_pool2')
                mifc2 = tflearn.fully_connected(net, 1024, activation='tanh', name='middle_fc2')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='middle_conv3_1_3x3')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='middle_conv3_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='middle_pool3')
                net=tflearn.fully_connected(net, 1024, activation='tanh', name='middle_fc1')
                net=tf.concat([net, mifc2], 1, name='middle_fc')
                net=tflearn.dropout(net, 0.8)
            else:
                print("UNVALID INPUT ")
                exit(-1)
            ...
            with tf.Session() as sess:
                ...
                for i in range(iters):
                    batch=traindata.next_batch(batchSize, shuffle=False)
                    _, tloss=sess.run([optimizer, loss], feed_dict={x:batch[3],y:batch[5]})
                    if tloss<mini_loss:
                        mini_loss=tloss
                    v_accuracy, valid_loss, oaa, confu_mat = Valid_on_TestSet(sess, accuracy, sum_test, loss, softmax,
                                                              x, testdata.middlep, y, testdata.labels)
                    laflag=loss_a.analyzeLossVariation(valid_loss)
                    clr=cLR*(lr_drate)**(i//lrstep)
                    tt=time.time()
                    print("CLR:%.8f Ite:%06d Bs:%03d Epo:%04d Los:%.8f mLo:%08f\tVALID>> mVL: %.8f\tVL: %.8f\tVA: %f\tOAA: %f\tT: %fs"%
        ...
        #
            ...
            if NetworkType==4:#4
                ...
                net=tflearn.conv_2d(net, 32, 3, activation='relu', name='mouth_conv2_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='mouth_pool2')
                mfc2 = tflearn.fully_connected(net, 1024, activation='tanh', name='mouth_fc2')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='mouth_conv3_1_3x3')
                net=tflearn.conv_2d(net, 128, 3, activation='relu', name='mouth_conv3_2_3x3')
                net=tflearn.max_pool_2d(net, 2, 2, name='mouth_pool3')
                net=tflearn.fully_connected(net, 1024, activation='tanh', name='mouth_fc1')
                net=tf.concat([net, mfc2], 1, name='mouth_fc')
                net=tflearn.dropout(net, 0.8)
            ...
            with tf.Session() as sess:
                ...
                for i in range(iters):
                    batch=traindata.next_batch(batchSize, shuffle=False)
                    _, tloss=sess.run([optimizer, loss], feed_dict={x:batch[4],y:batch[5]})
                    if tloss<mini_loss:
                        mini_loss=tloss
                    v_accuracy, valid_loss, oaa, confu_mat = Valid_on_TestSet(sess, accuracy, sum_test, loss, softmax,
                                                              x, testdata.mouthp, y, testdata.labels)
                    laflag=loss_a.analyzeLossVariation(valid_loss)
                    clr=cLR*(lr_drate)**(i//lrstep)
                    tt=time.time()
                    print("CLR:%.8f Ite:%06d Bs:%03d Epo:%04d Los:%.8f mLo:%08f\tVALID>> mVL: %.8f\tVL: %.8f\tVA: %f\tOAA: %f\tT: %fs"%
        ...
        tt=time.time()
        filelog=open(log,'a')
        filelog.write('%s\t\t TotalTimeConsumed: %f\n'%(file_record, (tt-t1)))
        filelog.close()
        print(log)
        print(log.split('.txt')[0])
        losslog=log.split('.txt')[0]+'_Runs%d'%(runs)+'.validationlosslist'
        losslog=losslog.replace('./logs/facepatchpretrain/','./logs/facepatchpretrain/VLL/')
        ...
        try:
            ...
            print('\n>>>>>> Current run info has been save after it crrupted or interrupted.\n\n')
        except:
